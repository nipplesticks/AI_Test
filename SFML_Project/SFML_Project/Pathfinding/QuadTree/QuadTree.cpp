...
Entity * QuadTree::DispatchRay(const sf::Vector2f & rayStart, const sf::Vector2f & rayEnd, std::vector<unsigned long long> * avoid) const
{
	Entity * e = nullptr;
	float t = FLT_MAX;
	_traverseWithRay(rayStart, rayEnd, 0, t, e, avoid);
	return e;
}
...
void QuadTree::_traverseWithRay(const sf::Vector2f & rayStart, const sf::Vector2f & rayEnd, int quadIndex, float & t, Entity *& ePtr, std::vector<unsigned long long> * avoid) const
{
	if (t > 0.0f)
	{
		if (_insideRay(rayStart, rayEnd, m_quadTree[quadIndex]))
		{
			...
			if (nrOfChildren > 0)
			{
				const unsigned int * children = m_quadTree[quadIndex].GetChildren();
				for (int i = 0; i < nrOfChildren; i++)
					_traverseWithRay(rayStart, rayEnd, children[i], t, ePtr, avoid);
			}
			else
			{
				// Trace Objects
				...
				for (int i = 0; i < size; i++)
				{
					if (_insideRay(rayStart, rayEnd, objects[i], tTemp) && tTemp < t)
					{
						if (avoid)
						{
							auto it = std::find(avoid->begin(), avoid->end(), objects[i]->GetUniqueID());
							if (it == avoid->end())
							{
								ePtr = objects[i];
								t = tTemp;
